...
function renderHome() {
  ...
  const html = `
    <div class="intro">
      <h2>项目简介</h2>
      <p>本系统用于展示中药材种植地的土壤重金属含量和药材重金属含量数据。</p>
      <p>数据包含${allData.length}个采样点，涵盖${herbs.length}种中药材。</p>
      <p>点击下方卡片查看各中药材的详细信息，或访问"完整数据表"查看所有数据。</p>
    </div>
    <div class="card-grid">
      ${herbs.map(herb => `
...
function renderOnlineMap(data) {
  const container = document.getElementById('leafletMap');
  container.innerHTML = '<div class="empty-state">在线地图功能需要 Leaflet 库支持。<br>当前使用离线模式，请切换到"离线经纬度散点图"。</div>';

  console.log('在线地图功能暂未实现，请使用离线散点图');
}
